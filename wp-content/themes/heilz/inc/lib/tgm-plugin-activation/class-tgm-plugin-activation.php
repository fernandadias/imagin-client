...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function notices() {
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					$rendered = '<span class="ulz-db-settings-error-heading">' . esc_html__( 'Install / Activation / Update of required plugin(s) needed', 'heilz' ) . '</span>';
				...
				$rendered = '<span class="ulz-outer" style="display: block;">' . $rendered . '</span>';
				$rendered .= '<a class="ulz-dismiss" href="' . esc_url( wp_nonce_url( add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ), 'tgmpa-dismiss-' . get_current_user_id() ) ) . '"></a>';
		...
		protected function create_user_action_links_for_notice( $install_count, $update_count, $activate_count, $line_template ) {
			...
			$link_template = '<a class="ulz--button ulz--secondary" href="%2$s">%1$s</a>';
		...
		protected function get_admin_notice_class() {
			if ( ! empty( $this->strings['nag_type'] ) ) {
				return 'ulz-db-notice ' . sanitize_html_class( strtolower( $this->strings['nag_type'] ) );
			} else {
				if ( version_compare( $this->wp_version, '4.2', '>=' ) ) {
					return 'notice-error ulz-db-notice';
				} elseif ( version_compare( $this->wp_version, '4.1', '>=' ) ) {
					return 'notice ulz-db-notice';
				} else {
					return 'updated ulz-db-notice';
				}
			}
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	 ...
	 *
	 * Gives users an interface related to the Plugin Administration
	 * area with related (albeit stripped down) capabilities.
	 *
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_row_actions( $item ) {
			...
			$prefix = ( defined( 'WP_NETWORK_ADMIN' ) && WP_NETWORK_ADMIN ) ? 'netwoulz_admin_' : '';
