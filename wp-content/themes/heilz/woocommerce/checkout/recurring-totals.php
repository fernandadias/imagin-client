...
<tr class="recurring-totals">
	<th colspan="2"><?php esc_html_e( 'Recurring totals', 'heilz' ); ?></th>
</tr>
...
<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
	...
	<tr class="cart-subtotal recurring-total">
		...
		if ( $display_th ) :
			$display_th = false;
			?>
			<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Subtotal', 'heilz' ); ?></th>
			<td data-title="<?php esc_attr_e( 'Subtotal', 'heilz' ); ?>"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></td>
		<?php else : ?>
...
<tr class="order-total recurring-total">
	...
	if ( $display_th ) :
		$display_th = false;
		?>
		<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Recurring total', 'heilz' ); ?></th>
		<td data-title="<?php esc_attr_e( 'Recurring total', 'heilz' ); ?>"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></td>
	<?php else : ?>
